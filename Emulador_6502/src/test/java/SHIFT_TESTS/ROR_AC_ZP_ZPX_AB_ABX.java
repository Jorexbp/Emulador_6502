...
public class ROR_AC_ZP_ZPX_AB_ABX {
	...
	public static Collection<Object[]> data() {
		Object[][] data = new Object[][] { { OPCODES.INS_ROR_AC.opcodeValue, 2 }, { OPCODES.INS_ROR_ZP.opcodeValue, 5 },
				{ OPCODES.INS_ROR_ZPX.opcodeValue, 6 }, { OPCODES.INS_ROR_AB.opcodeValue, 6 },
				{ OPCODES.INS_ROR_ABX.opcodeValue, 7 } };
	...
	public void test() {
		...
		if (OPCODES.INS_ROR_AC.opcodeValue == OPCODE) {
			assertEquals(CPU.A, 0x01);
			assertEquals(cpu.N, false);
			assertEquals(cpu.Z, false);
			assertEquals(cpu.C, false);
		} else if (OPCODES.INS_ROR_ZP.opcodeValue == OPCODE) {
			assertEquals(CPU.mem.data[0x0042], 0x03);
			assertEquals(cpu.N, false);
			assertEquals(cpu.Z, false);
			assertEquals(cpu.C, false);
		} else if (OPCODES.INS_ROR_ZPX.opcodeValue == OPCODE) {
			assertEquals(CPU.mem.data[0x0043], 0x03);
			assertEquals(cpu.N, false);
			assertEquals(cpu.Z, false);
			assertEquals(cpu.C, false);
		} else if (OPCODES.INS_ROR_AB.opcodeValue == OPCODE) {
			assertEquals(CPU.mem.data[0x1242], 0x03);
			assertEquals(cpu.N, false);
			assertEquals(cpu.Z, false);
			assertEquals(cpu.C, false);
		} else if (OPCODES.INS_ROR_ABX.opcodeValue == OPCODE) {
